...
struct sort_container{
	int idx;
	double value;
};

bool operator<(const sort_container& left, const sort_container& right)
{
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    ...
    // define variables    
    vector<double> bestSSE( selectFeatureDim, DBL_MAX );
    vector<double> bestThr( selectFeatureDim, 0 );
    vector<vector<double>> rightOutput( selectFeatureDim );
    for( int i=0; i<selectFeatureDim; i++ ){
        rightOutput[i].resize( targetDim, 0 );
    }
    vector<vector<double>> leftOutput( selectFeatureDim );
    for( int i=0; i<selectFeatureDim; i++ ){
        leftOutput[i].resize( targetDim, 0 );
    }
    
    vector<double> bestLeftSSE( selectFeatureDim, 0 );
    vector<double> bestRightSSE( selectFeatureDim, 0 );
    
    //////////////////////////////////////
    vector<int> randIdxes( featureDim );
    for( int i=0; i<featureDim; i++ ){
        randIdxes[i] = i;
    }
    random_shuffle ( randIdxes.begin(), randIdxes.end() );
...
#pragma omp parallel for schedule(dynamic )
    ...
    for( int i=0; i<selectFeatureDim; i++ ){
        ...
        vector<sort_container> sortByValue;
        sortByValue.resize( dataNum );
        for( int j=0; j<dataNum; j++ ){
            sortByValue[j].idx = j;
            sortByValue[j].value = feature(j,m);
        }
        sort(sortByValue.begin(),sortByValue.end());
        
        // calc initial average
        vector<double> aveLeft( targetDim, 0 );
        vector<double> aveRight( targetDim, 0 );
        vector<double> aveLeftPre( targetDim, 0 );
        vector<double> aveRightPre( targetDim, 0 );
        ...
        for( int j=0; j<dataNum-1; j++ ){
            ...
            for( int k=0; k<targetDim; k++ ){                
                sseLeft = sseLeft + ( target(sortByValue[j].idx,k) - aveLeftPre[k] ) * ( target(sortByValue[j].idx,k) - aveLeft[k] );                
                sseRight = sseRight - ( target(sortByValue[j].idx,k) - aveRightPre[k] ) * ( target(sortByValue[j].idx,k) - aveRight[k] );                
            }
    ...
    int finalIdx;
    double finalThr;
    vector<double> finalLeftOutput( targetDim, 0 );
    vector<double> finalRightOutput( targetDim, 0 );
